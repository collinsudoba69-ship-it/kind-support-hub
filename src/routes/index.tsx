...
import { Copy, ExternalLink, Pencil, Plus, ShieldCheck, Trash2, Clock } from "lucide-react";
...
import { loadSites, saveSites, type SupportSite } from "@/lib/sites";
...
function Index() {
  ...
  function persist(next: SupportSite[]) {
    setSites(next);
    saveSites(next);
  }

  function handleSave(site: SupportSite) {
    const exists = sites.some((s) => s.id === site.id);
    persist(exists ? sites.map((s) => (s.id === site.id ? site : s)) : [site, ...sites]);
  }
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <div className="relative mx-auto w-full max-w-3xl px-5 pb-24 pt-10 sm:pt-16">
        ...
        <section className="mt-16" aria-labelledby="your-sites">
          <h2
            id="your-sites"
      ...
      <SupportWizard open={open} onOpenChange={setOpen} onSave={handleSave} editing={editing} />
    </main>
  );
}
